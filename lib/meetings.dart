...
class Meetings extends StatelessWidget {
  ...
  Widget _buildMeetingItem(BuildContext context, int index) {
    ...
    var meetingDay =DateFormat.yMd().format(DateTime.parse(meetings[index].startTime));
    var isToday = today.compareTo(meetingDay);

    var isUpcoming = DateTimeFormatter.isUpcomingMeeting(meetings[index].startTime);
    return Container(
          ...
          child: Column(
            children: <Widget>[
              Container(
                ...
                decoration: new BoxDecoration(
                  color: (isToday == 0 || isUpcoming == true)? (isUpcoming == true ? Colors.blue : Colors.green) : Colors.grey,
                  border: Border.all(
                    color: Colors.black12, width: 1.0
                  ),
                  borderRadius: BorderRadius.circular(20.0)
                ),
                child: (isToday == 0 || isUpcoming == true ) ? (isUpcoming == true ? Text(
                  'Upcoming',
                  style: TextStyle(fontSize: 12.0,color: Colors.white),
                ):
                Text('Today', style: TextStyle(fontSize: 12.0,color: Colors.white),)
              ):Text('${DateTimeFormatter.getDate(meetings[index].startTime)}')
              ),
              Text(
                meetings[index].title,
